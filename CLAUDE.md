...
| 2026-05-25 | `ansible/roles/caddy/tasks/main.yml` | Sudo requires a password (no NOPASSWD) | Narrow NOPASSWD for `systemctl reload caddy` and `systemctl restart caddy` only.  Required so the login user can apply Caddy config changes (demo-sites.d workflow) without direct admin API socket access — the socket is restricted to caddy/root to prevent CA cert and config exfiltration via the Caddy admin API. |
